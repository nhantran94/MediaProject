...
# Add to the -XaddExports: option which can only occur once
AddVMAddExportsOption()
{
  if [ "$USING_JDK9_JIGSAW" = "0" ]
  then
    EchoIfVerbose "Adding VM_ADDEXPORTS Option: $*"
    APP_VM_ADDEXPORTS_OPTS[${#APP_VM_ADDEXPORTS_OPTS[*]}]="$*"
  else
    EchoIfVerbose "Skipping VM_ADDEXPORTS Option because JDK9 is not jigsaw enabled: $*"
  fi
}
#-----------------------------------------------------------------------------
...
AppendVMSpecificOptions()
{
  ...
  if [ $(expr "${APP_JAVA_MAJOR_VERSION}" \>= "1.9" ) -eq 1 ]
  then
    EchoIfVerbose "Size of add exports is ${#APP_VM_ADDEXPORTS_OPTS[@]}"
    if [ $(expr "${#APP_VM_ADDEXPORTS_OPTS[@]}" \> "0" ) -eq 1 ]
    then
      local addExportsOption="-XaddExports:"
      local oneComma=""
      for (( i = 0; i < ${#APP_VM_ADDEXPORTS_OPTS[@]}; i++ )); do
        addExportsOption="$addExportsOption$oneComma${APP_VM_ADDEXPORTS_OPTS[$i]}"
        oneComma=","
      done
      AddVMOption "$addExportsOption"
    fi
...
LaunchIDE()
{
    ...
    # Assume we're using a jigsaw enabled JDK9
    USING_JDK9_JIGSAW=0
        ...
        case $1 in
          ...
          --*)
            ...
            case $NEWARG in
              ...
              addvmaddexportsoption)   NEWARG="AddVMAddExportsOption" ;;
              *) NEWARG="" ;;
            esac
            if [ ! "$NEWARG" = "" ]
            then
